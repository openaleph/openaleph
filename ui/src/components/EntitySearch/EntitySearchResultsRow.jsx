      ...
      <tr className={c('EntitySearchResultsRow', 'nowrap', 'skeleton')}>
        ...
        {columns.map(({ name, type }) => (
          <td key={name} className={type?.name || name}>
            <Skeleton.Text
              type="span"
              length={name === 'caption' || name === 'collection_id' ? 30 : 15}
            />
          </td>
        ))}
      </tr>
    ...
    if (name === 'caption') {
      ...
      return (
        <Property.Values
          prop={{
            name,
            type: { name: type, values: model.types[type]?.values },
          }}
          values={entity.getProperty(name)}
          missing="-"
          truncate={1}
          truncateItem={30}
        />
      ...
      return (
        <Property.Values
          prop={{ type: { name: key, values: model.types[key]?.values } }}
          values={values}
          missing="-"
          truncate={1}
          truncateItem={30}
        ...
        <tr key={entity.id} className={resultClass}>
          ...
          {columns.map((column) => {
            ...
            return (
              <td
                key={column.name}
                className={column.type?.name || column.name}
              >
                {content}
              </td>
            );
