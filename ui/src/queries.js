...
export function canonicalSimilarQuery(location, canonicalId) {
  const path = canonicalId ? `canonical/${canonicalId}/similar` : undefined;
  return Query.fromLocation(path, location, {}, 'similar').defaultFacet(
    'collection_id',
    true
  );
...
export function canonicalExpandQuery(canonicalId, properties, limit = Query.LARGE) {
  const context = { 'filter:property': properties };
  const path = canonicalId ? `canonical/${canonicalId}/expand` : undefined;
  return new Query(path, {}, context, 'expand').limit(limit);
}

export function canonicalReferencesQuery(canonicalId) {
  return canonicalExpandQuery(canonicalId, [], 0);
}

export function canonicalReferenceQuery(location, canonical, reference) {
  const context = {
    [`filter:properties.${reference?.property?.name}`]: canonical?.merged?.referents,
    'filter:schemata': reference?.schema,
  };
  const path =
    canonical?.id && reference?.schema && reference?.property
      ? 'entities'
      : undefined;
  return Query.fromLocation(path, location, context, reference?.property?.name);
}
